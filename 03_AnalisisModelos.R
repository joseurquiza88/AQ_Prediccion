...
lm_model <- lm(PM25 ~ AOD_055,
               data = train_data)
resultados_RLS <- evaluar_modelo(lm_model, test_data)
...
resultados_RLM <- evaluar_modelo(lm_model, test_data)
...
svr_model <- svm(PM25 ~ AOD_055 + ndvi + BCSMASS_dia + DUSMASS_dia + 
                   ...
                   d2m_mean  + v10_mean + u10_mean + tp_mean +  dayWeek,
                 data = train_data,
                 type = "eps-regression",  # regresión epsilon-SVR
                 kernel = "radial",        # podés probar también "linear" o "polynomial"
                 cost = 10,                # parámetro de penalización (ajustable)
                 epsilon = 0.1)            # margen de tolerancia (ajustable)

resultados_SVR <- evaluar_modelo(svr_model, test_data)
...
# Entrenamiento simple sin búsqueda de hiperparámetros ni CV
modelo_ranger <- train(
  ...
  tuneGrid = data.frame(
    mtry = 5,                # elegí un valor fijo, por ejemplo 5
    splitrule = "extratrees", # Extra Trees
    min.node.size = 5        # también podés cambiar este valor si querés
  ),
  importance = 'impurity'
)

resultados_ET <- evaluar_modelo(modelo_ranger, test_data)
print(resultados_ET)
#########################################################################################
#########################################################################################
### ----- RF   -----
...
resultados_RF <- evaluar_modelo(modelo_RF, test_data)
print(resultados_RF)
#########################################################################################
#########################################################################################
